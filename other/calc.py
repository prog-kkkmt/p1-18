...
def calculate():
    ...
    try:
        ...
        messagebox.showinfo('Attention', '0 / 0 не делится')
        calc.insert(0, 0)
        calc['state'] = tk.DISABLED
...
def press_key(event):
    ...
    elif '+' in event.char or '-' in event.char or '*' in event.char or '/' in event.char:
        add_operation(event.char)
    elif '\r' in event.char or '=' in event.char:
        calculate()
    elif event.char == '\x08':
        backspace()
