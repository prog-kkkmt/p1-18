...
class Update(Child):
    ...
    def edit(self):
        ...
        btn_edit.bind('<Button-1>', lambda event: self.deletepop())

        self.btn_ok.destroy()

    def deletepop(self):
        self.view.update_records(self.entry_equipid.get(),
                                 self.entry_equiptypeid.get(),
                                 self.entry_name.get(),
                                 self.entry_dayof.get(),
                                 self.entry_note.get())
...
class Database():
    def __init__(self):
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS equip(equipid INT PRIMARY KEY AUTOINCREMENT, equiptypeid INT, equipname TEXT, dayof TEXT, note TEXT,
                            FOREIGN KEY(equiptypeid) REFERENCE equiptype(equiptypeid))''')
        self.conn.commit()
