...
def insert_data():
    ...
    db.cursor.execute("INSERT INTO orders (order_id, order_name, customer_name, number, price) VALUES (?,?,?,?,?)",
                      (window.id_order.get(), window.name_order.get(), window.customer.get(), window.number.get(), window.price.get()))
    db.save()
...
class Window:
    ...
    def top_menu(self):
        ...
        self.menu.add_cascade(label="О программе", command=lambda: messagebox.showinfo("О программе", "Разработку ведет Батраков Данила <3. "))  # Меню о программе добавляется на панель меню
    ...
    def main_menu(self):
        ...
        self.create_label("Заказ: ", 0, 40)
        Entry(width=20, textvariable=self.name_order).place(x=0, y=60)

        self.create_label("Заказчик:", 160, 3)
        Entry(width=20, textvariable=self.customer).place(x=160, y=20)

        self.create_label("Номер телефона:", 160, 40)
        Entry(width=20, textvariable=self.number).place(x=160, y=60)

        self.create_label("Цена: ", 320, 3)
        Entry(width=20, textvariable=self.price).place(x=320, y=20)
        ...
        Entry(width=20, textvariable=self.id_order_del, ).place(x=480, y=20)

        Button(self.window, text=TEXT_BUTTON_DEL, width= 8, command=delete_order_by_id, bg=BUTTON_COLOR,
