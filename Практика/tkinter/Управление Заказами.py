...
def creat_db():  # диалоговое окно, где в поле имя файлов надо ввести имя файла которое вы хотите создать
    global nameDB
...
def open_db():  # диалоговое окно, где пользователь выбирает какой файл ему нужно открыть
    global nameDB
    path_db = filedialog.askopenfilename(initialdir="/", title="Select file", filetypes=(("database files", "*.db"), (
    "all files", "*.*")))  # op является путём и его надо передать в функции добавления и удаления из файла
    db = sqlite3.connect(path_db)
    nameDB = path_db
...
class Window:
    def __init__(self):
        ...
        self.id_order = StringVar()  # Присваивает строку из виджета поля для ввода текста
    ...
    def top_menu(self):
        self.menu = Menu(self.window)  # иницилизирую объект меню для секций
        self.menu.add_cascade(label=TEXT_CREATE_FILE, command=creat_db)
        self.menu.add_cascade(label=TEXT_OPEN_FILE, command=open_db)
        self.menu.add_cascade(label="О программе", command=lambda: messagebox.showinfo("О программе",
                                                                                       "Разработку ведет Батраков Данила."))  # Меню о программе добавляется на панель меню
    ...
    def create_label(self, text_label, x_label, y_label):  # Функция принимает строку и координаты
        Label(text=text_label, bg=LABEL_BG).place(x=x_label,
                                                  y=y_label)  # Отображает текст в окне по переданным координатам

    def main_menu(self):
        self.create_label("ID заказа: ", 0, 3)  # Отображаем текст хранящейся в переменной
        Entry(width=20, textvariable=self.id_order).place(x=0,
                                                          y=20)  # Определим элемент Entry который представляет собой поле для ввода текста
