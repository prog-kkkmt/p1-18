...
class HelpCashier(tk.Toplevel):
	def __init__(self, parent):
	    ...
	    # Имя документа
	    self.doc = StringVar()
	    lbl_doc = tk.Label(self, text="Имя документа:")\
                        .grid(row=0, column=0, sticky=W, pady=10, padx=10)
	    ent_doc = tk.Entry(self, textvariable=doc)\
                        .grid(row=0, column=1, columnspan=3, sticky=W+E, padx=10)

	    # Оплата клиента
	    self.money = IntVar()
	    lbl_money = tk.Label(self, text="Оплата клиента:")\
                          .grid(row=1, column=0, sticky=W, pady=10, padx=10)
	    ent_money = tk.Entry(self, textvariable=money)\
                          .grid(row=1, column=1, columnspan=3, sticky=W+E, padx=10)

	    # Нал/Безнал
	    self.payment = StringVar()		
	    lbl_payment = tk.Label(self, text="Вид оплаты (нал/безнал):")\
                            .grid(row=2, column=0, sticky=W, pady=5, padx=10)
	    ent_payment = tk.Entry(self, textvariable=payment, width=30)\
                            .grid(row=2, column=1, columnspan=3, sticky=W+E, padx=10)

	    # Кнопки
	    btn_canel = tk.Button(self, text='Отменить', bg='red', command=self.destroy)\
                            .grid(row=3, column=2, pady=10)
	    btn_submit = tk.Button(self, text='Принять', bg='lightgreen', command=self.accept_data)\
